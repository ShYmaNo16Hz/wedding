...
const CHUNK_SIZE = 8 * 1024 * 1024; // 8MB — trebuie multiplu de 256KB
const MAX_RETRIES = 5;
// ─────────────────────────────────────────────────────────────────────────────
...
export default {
  ...
  methods: {
    ...
    // ── Upload direct din browser → Google Drive (pe bucăți, cu reluare) ──────
    async uploadFile(item) {
      const file = item.file;
      const mimeType = file.type || "application/octet-stream";

      // 1. Cerem sesiunea de upload de la server (payload mic, doar JSON)
      ...
      // 2. Upload pe bucăți, cu reluare automată dacă pică rețeaua
      let offset = 0;
        ...
        while (!succeeded) {
          try {
            ...
            if (result.done) {
              ...
              return; // upload complet
            }
            offset = result.nextOffset;
            this.currentFileProgress = (offset / file.size) * 100;
            succeeded = true;
          } catch (e) {
            attempt++;
            if (attempt > MAX_RETRIES) {
              throw new Error("Upload eșuat după mai multe încercări");
            }
            // aflăm exact de unde a rămas Google (poate a primit parțial bucata)
      ...
      return new Promise((resolve, reject) => {
        ...
        xhr.upload.onprogress = (e) => {
          if (e.lengthComputable) {
            this.currentFileProgress = ((start + e.loaded) / totalSize) * 100;
          }
        };
        ...
        xhr.onload = () => {
          if (xhr.status === 200 || xhr.status === 201) {
            ...
            // bucată acceptată; Google confirmă până unde a primit
            const range = xhr.getResponseHeader("Range"); // ex: "bytes=0-8388607"
      ...
      return new Promise((resolve) => {
        ...
        xhr.onload = () => {
          if (xhr.status === 308) {
            ...
            resolve(totalSize); // deja complet, dintr-o tentativă anterioară
          } else {
            resolve(0);
          }
        };
